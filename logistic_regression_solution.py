...
def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    ...
    # second cluster is X2
    X2 += np.array([distance, distance]) # need to shift along x and y axes
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        Y_test_prob = model.predict_proba(X)[:, 1]
        loss_list.append(log_loss(y, Y_test_prob))
        ...
        x_vals = np.linspace(x_min, x_max, 200)  # Generate x1 values
        decision_boundary = slope * x_vals + intercept  # Compute corresponding x2 values
        plt.plot(x_vals, decision_boundary, 'k--', label='Decision Boundary', linewidth=2)
